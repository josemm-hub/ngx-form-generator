...
export function makeForm(spec: OpenAPI.Document): string {
  ...
  const forms = definitionKeys
    .map(key => (definitions?.[key] ? makeDefinition(key, definitions?.[key]) : ''))
    .filter(item => item != '')
    .join('');
...
function makeFieldsBody(definition: Definition): string[] {
  ...
  const fieldsBody = fields
    .map(fieldName =>
      makeField(fieldName, definition.properties?.[fieldName], definition.required?.includes(fieldName))
    )
    .filter(item => item !== '');
 ...
 */
function makeField(fieldName: string, property: SchemaProperty, isRequired: boolean): string {
  let fieldRepesentation;
  if (property.allOf) {
    fieldRepesentation = makeField(fieldName, property.allOf, isRequired);
  } else if (property.type === 'array') {
    fieldRepesentation = makeArrayField(fieldName, property as OpenAPIV3.ArraySchemaObject);
  } else if (property.type === 'object') {
    fieldRepesentation = makeObjectField(fieldName, property);
  } else {
    fieldRepesentation = makePrimitiveField(fieldName, property);
  }
 ...
 */
function makeArrayField(fieldName: string, property: OpenAPIV3.ArraySchemaObject): string {
 ...
 */
function makeObjectField(fieldName: string, property: SchemaProperty): string {
  const groupBody = makeFieldsBody(property);
  return `"${fieldName}": new FormGroup({${groupBody}})`;
}
 ...
 */
function makePrimitiveField(fieldName: string, property: SchemaProperty): string {
  const value = 'default' in property ? `'${property.default}'` : null;
  return `"${fieldName}": new FormControl(${value}, [${makeFieldRules(fieldName, property)}])`;
}
 ...
 */
function makeFieldRules(fieldName: string, property: SchemaProperty): string {
  return rules
    .map(rule => rule(fieldName, property))
    .filter(item => item != '')
    .join();
}
